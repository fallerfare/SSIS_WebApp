...
def delete_students(id_number):
    ...
    return jsonify({
        "success": True,
        "message": f"Student {id_number} deleted successfully"
    }), 200
...
def delete_programs(program_code):
    ...
    try:
        ...
        if constraint_name == "fk_student_program":
            message = "This program can not be deleted at the moment as there are still students enrolled under it!"
            print(message)
        else:
...
def delete_colleges(college_code):
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"College {college_code} deleted successfully"
        }), 200
